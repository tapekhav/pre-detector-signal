...
EncodeWord::EncodeWord(double num)
{
    _bitset_num = *reinterpret_cast<unsigned long long*>(&num);
}

std::bitset<14> EncodeWord::getBitset(std::bitset<48> encoded_value, int &offset) const {
    std::bitset<14> value;
    uint8_t k = 0;
...
std::vector<std::bitset<14>> EncodeWord::getResultBitset() const
{
